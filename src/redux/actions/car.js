...
export const addCar =
    (navigate, name, photo, rentperday, cartype_id, setIsLoading) => async (dispatch, getState) => {
        const { token } = getState().auth;
...
export const updateCar = (id, navigate, setIsLoading, name, photo, rentperday, cartype_id) => async (dispatch, getState) => {
    const { token } = getState().auth;
    setIsLoading(true);
